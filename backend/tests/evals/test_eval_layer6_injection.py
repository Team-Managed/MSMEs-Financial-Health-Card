...
def test_adversarial_content_is_xml_fenced(adv_chunk):
    ...
    def _capture_generate(prompt: str):
    ...
    mock_model.generate_content.side_effect = _capture_generate

    with patch("backend.app.graph.nodes._get_gemini_model", return_value=mock_model):
        node_weight_setter({
            "profile": profile,
