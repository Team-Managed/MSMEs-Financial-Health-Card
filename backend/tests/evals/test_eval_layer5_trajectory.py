...
def _mock_agent_explain():
    """Return a patcher + strict-JSON response for the current Gemini adapter."""
    mock_response = MagicMock()
    mock_response.text = json.dumps({"narrative": "CFCR baseline pass.", "claims": []})
    patcher = patch("backend.app.graph.nodes._get_gemini_model")
...
def test_all_nodes_produce_expected_state_keys(persona_id):
    ...
    with patcher as mock_model_fn:
        mock_model_fn.return_value.generate_content.return_value = mock_response
    ...
    expected_keys = {
        ...
        "explainer_chunks",  # explainer_retriever
        "narrative",         # explainer/grounding_validator
        "claims",            # explainer/grounding_validator
