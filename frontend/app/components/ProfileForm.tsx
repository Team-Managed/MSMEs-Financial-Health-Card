...
interface FormState {
  sector: string;
  years: string;
  turnoverRange: "micro" | "small" | "medium" | "";
  gstRegistered: "yes" | "no" | "";
  employeeTier: "micro" | "small" | "medium" | "";
  hasLoans: "yes" | "no" | "";
...
type Action =
  ...
  | {
      field: "turnoverRange" | "employeeTier";
      value: "micro" | "small" | "medium";
    }
  | {
      field: "gstRegistered" | "hasLoans" | "highConcentration" | "seasonal";
      value: "yes" | "no";
    };
  ...
  return (
    <div className="flex gap-2">
      {options.map((opt) => (
        <label
          key={opt.value}
          className={`flex flex-1 cursor-pointer items-center justify-center rounded-md border px-3 py-2.5 text-sm font-medium transition-colors ${
            value === opt.value
...
export default function ProfileForm({ onSubmit, loading }: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    onSubmit({
      sector: form.sector,
      yearsOperating: parseInt(form.years, 10),
      profileType: deriveProfileType(form),
      msmeTier: form.turnoverRange as "micro" | "small" | "medium",
      gstRegistered: form.gstRegistered === "yes",
      employeeTier: form.employeeTier as "micro" | "small" | "medium",
    });
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-7">
      <div>
        <h2 className="text-base font-semibold text-slate-800">
          MSME Credit Health Assessment
        </h2>
        <p className="mt-1 text-xs text-slate-400">
          Fill in your business details for a stress-tested financial health
          analysis.
        </p>
      </div>
      ...
      <fieldset className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <input
            ...
            value={form.years}
            onChange={(e) =>
              dispatch({ field: "years", value: e.target.value })
            }
            className="w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
          />
        </div>

        {/* Turnover — MSME classification */}
        <div className="space-y-2">
          ...
          <RadioGroup
            ...
            options={[
              { value: "micro", label: "< ₹5 Cr · Micro" },
              { value: "small", label: "₹5–50 Cr · Small" },
              { value: "medium", label: "₹50–250 Cr · Medium" },
            ]}
            onChange={(v) => dispatch({ field: "turnoverRange", value: v })}
          />
          <p className="text-xs text-slate-400">
            As per MSMED Act 2020 — determines loan eligibility tier
          </p>
        </div>
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-slate-400">
            GST data is a primary alternate-data signal for credit assessment
          </p>
          <RadioGroup
            name="gstRegistered"
            ...
            onChange={(v) => dispatch({ field: "gstRegistered", value: v })}
          />
        </div>
      </fieldset>
      ...
      <fieldset className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-slate-400">
            Used to assess EPFO payroll consistency
          </p>
          <RadioGroup
            name="employeeTier"
            ...
            onChange={(v) => dispatch({ field: "employeeTier", value: v })}
          />
        </div>
      </fieldset>
          ...
          [
            {
              ...
              label: "Existing bank loans or EMIs?",
              hint: "Determines whether repayment history is available as a scoring signal",
            },
            {
              field: "highConcentration" as const,
              label: "Single customer > 50% of revenue?",
              hint: "High buyer concentration increases vulnerability to shock scenarios",
            },
            {
              field: "seasonal" as const,
              label: "Revenue varies significantly by season?",
      ...
      >
        {loading ? "Running analysis…" : "Run Health Assessment →"}
      </button>
    </form>
  );
}
